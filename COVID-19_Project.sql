...
--- Overview of the Data that will be used focusing on Covid-19_Deaths ---
...
/* Likelyhood of dying from COVID-19 in 2023. (Sample Country: Philppines) */
SELECT location, date, total_cases, total_deaths,
	ROUND((CONVERT(FLOAT, total_deaths) / NULLIF(CONVERT(FLOAT, total_cases), 0)) * 100,5) as DeathPerCase
FROM [Covid-19_Portfolio Project]..[Covid-19_Deaths]
...
/* Likelyhood of contracting from COVID-19 in highly populated areas. */
...
/* COVID-19 Global Death Percentage within the Date Range of January 1, 2020, to October 5, 2023. (Using CTEs) */
WITH GlobalNumbers AS (
    SELECT 
        CONVERT(FLOAT, SUM(CASE WHEN ISNUMERIC(new_deaths) = 1 THEN CAST(new_deaths AS FLOAT) ELSE 0 END)) AS total_deaths,
        CONVERT(FLOAT, SUM(CASE WHEN ISNUMERIC(new_cases) = 1 THEN CAST(new_cases AS FLOAT) ELSE 0 END)) AS total_cases
